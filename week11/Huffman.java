public class Huffman {
	...
	private void insert(HuffmanEntry temp) {
		// TODO Auto-generated method stub

	}

	private HuffmanEntry deleteMin() {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public void insert1(HuffmanEntry newEntry) {
		a[++N] = newEntry; // 새로운 키를 배열 마지막 원소 다음에 저장
		upheap1(N); // 위로 올라가며 힙 속성 회복시키기 위해
	}

	private void upheap1(int n2) {
		// TODO Auto-generated method stub

	}

	public HuffmanEntry deleteMin1() {
		HuffmanEntry max = a[1]; // a[1]의 최대값을 max로
		swap1(1, N--); // 힙의 마지막 항목과 교환하고 힙 크기를 1 감소
	...
	private void downheap1(int i) { // i는 현재 노드의 인덱스
		while (2 * i <= N) { // i의 왼쪽 자식노드가 힙에 있으면
			...
			swap1(i, k); // 현재 노드가 자식 승자보다 작으면 현재 노드와 자식 승자와 교환
